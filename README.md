...
> **如果没有开启此选项，快捷指令会提示“此操作正尝试共享大量健康项目，这不受允许”，并且无法继续同步。**
>
> 请在 iPhone 或 iPad 上依次打开：
>
> **`设置 → 快捷指令 → 高级 → 允许共享大量数据`**

这是 iOS 对批量读取和共享健康数据的系统级保护开关，无法由快捷指令自动开启。操作路径也可参考 [Apple 官方说明](https://support.apple.com/zh-cn/guide/shortcuts/apd961a4fc65/ios)。

## 功能

- UI 配置：每台 iPhone/iPad 创建独立的随机 webhook。
- webhook 强制 `local_only=True`，只接受 `POST`/`PUT`。
- 内置健康指标在集成启动时创建传感器实体；未收到数据时显示为不可用，首次上传后立即更新。
- 固定创建 Wi-Fi 详情、上次同步、位置追踪、“显示连接信息”和“重置健康数据来源”按钮实体。
- 保存最后一次数据，HA 重启后仍可恢复。
...
1. **先在 iOS 中打开 `设置 → 快捷指令 → 高级 → 允许共享大量数据`。这是必做步骤。**
2. 打开“设置 → 设备与服务 → 添加集成”，搜索 `Apple Health Bridge` 或“苹果健康桥接”。
3. 输入设备名称。
4. 从 HA 持久通知中复制本地 webhook 地址。
5. 下载并打开 [已签名的 Apple Health Bridge 快捷指令](shortcut/dist/Apple%20Health%20Bridge.shortcut)。
6. 导入时粘贴完整 webhook 地址；之后不需要手工搭建任何动作。
7. 首次运行时，先在 iOS 的健康授权页确认 13 类健康数据访问，再从快捷指令读取到的真实 HealthKit 来源名称中选择设备，最后选择本次要同步的数据；这些选择会保存到 HA，后续自动化运行不会再次询问。更换设备或修改设备名称后，按 HA 中的“重置健康数据来源”按钮即可重新选择。
...
> 修复说明：健康样本筛选写入的是快捷指令保存的内部枚举值（例如 `Steps`、`Active Calories`），不是界面显示的中文文本；因此中文系统也能正确解析健康类型。
