    ...
    async def async_update(
        ...
        *,
        ...
        if wifi_available is not None:
            # The main Shortcut request reports Wi-Fi state on every run.
            # Clear cached values first so a cellular sync makes both Wi-Fi
            # entities unavailable instead of retaining the old network.
            self.data["wifi"] = {}
